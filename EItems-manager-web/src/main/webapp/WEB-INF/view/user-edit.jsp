...
<div style="padding:10px 10px 10px 10px">
	<form id="itemAddForm" class="itemForm" method="post">
	    <table cellpadding="5">
	        <tr>
	            <td>用户名称:</td>
	            <td><input class="easyui-textbox" type="text" name="name" data-options="required:true" style="width: 280px;" value="${user.name}"></input></td>
	        </tr>
			<tr>
				<td>目前密码:</td>
			...
			<tr>
				<td>更改密码:</td>
				<td><input class="easyui-textbox" type="text" name="newPassword" data-options="required:true" style="width: 280px;" ></input></td>
			</tr>
	    </table>
...
<script type="text/javascript">
    ...
    function submitForm(){
        ...
        $.post("${pageContext.request.contextPath}/user/modify.do",$("#itemAddForm").serialize(), function(data){
            if(data.status == 200){
               $.messager.alert('提示',data.msg);
                clearForm();
				//window.location.replace("${pageContext.request.contextPath}/login.html")
                parent.location.href = "${pageContext.request.contextPath}/login.html";
            }
            if(data.status == 500){
                $.messager.alert('提示',data.msg);
            }
