...
$$('.bumpbox_gray').addEvent('click',function(e){
	e = new Event(e).stop();
	if($('grow') != null){
		$('grow').dispose();
	}
	...
	var el = new Element('div', {
		 'styles':{
			 ...
			 padding:'4px',
			 background:'#666 url(ajax-loader.gif) no-repeat center center',
			 left:middleH ,
			 top:middleV,
			 cursor:'pointer',
			 display:'block',
	...
	var eff2 = new Fx.Morph($('grow'), { transition: Fx.Transitions.Bounce.easeOut, duration: 100, wait:'link', onComplete:function(){
		el.dispose();
	}});
		
	var eff = new Fx.Morph($('grow'), { transition: Fx.Transitions.Bounce.easeOut, duration: 1000, wait:'link', onComplete:function(){
